...
describe("SMLYieldBond", () => {
  ...
  it("never lets an investor be repaid past their own cap even across many revenue events", async () => {
    const { token, bond, operator, investorA } = await deployFixture();

    await token.connect(investorA).approve(await bond.getAddress(), ethers.parseUnits("600", 6));
    const [, , , investorB] = await ethers.getSigners();
    await token.connect(investorB).approve(await bond.getAddress(), ethers.parseUnits("400", 6));
    await bond.connect(investorA).fund(ethers.parseUnits("600", 6));
    await bond.connect(investorB).fund(ethers.parseUnits("400", 6));

    const maxRepayment = (ethers.parseUnits("600", 6) * 11500n) / 10000n; // 115% of 600

    for (let i = 0; i < 20; i++) {
      await token.mint(operator.address, ethers.parseUnits("1000", 6));
      await token.connect(operator).approve(await bond.getAddress(), ethers.parseUnits("1000", 6));
      await bond.connect(operator).processRevenue(ethers.parseUnits("1000", 6));
    }

    expect(await bond.amountRepaid(investorA.address)).to.be.lte(maxRepayment);
  });
});
