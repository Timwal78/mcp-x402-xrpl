...
import type { AgentRole } from "./SwarmOrchestrator.js";

/**
 * BYOK LLM reasoning layer for the ASC swarm. This is what makes an agent
 * actually "decide" something instead of matching a string in a payload.
 * Each role gets its own system prompt and reasons over whatever context
 * the orchestrator gives it, returning a structured decision via forced
 * tool use so the orchestrator never has to parse free-form text.
 */
...
const ROLE_SYSTEM_PROMPTS: Record<AgentRole, string> = {
  CEO: `You are the CEO agent of an Autonomous Software Corporation (ASC) — a small, \
...
reasoning. You are reasoning over real operational context, not a hypothetical.`,
  ...
  CFO: `You are the CFO agent of an Autonomous Software Corporation (ASC). Your job is \
...
absurdly large relative to prior history, malformed), choose no_action and say why.`,
  ...
  CTO: `You are the CTO agent of an Autonomous Software Corporation (ASC). Your job is \
...
review. Keep proposals scoped and concrete; vague proposals get rejected by QA.`,
  ...
  QA: `You are the QA agent of an Autonomous Software Corporation (ASC) — the safety \
